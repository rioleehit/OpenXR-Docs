...
在每种用途中，API主要版本号，次要版本编号和补丁版本编号都包装到64位整数中，称为basetype：xrversion，如下：
...
* 主要版本编号是一个16位整数，该整数填充到63-48中。
* 次要版本编号是一个16位整数，该整数装在47-32中。
* 补丁版本号是一个32位整数，填充到31-0中。
****
...
// follows.
任何版本编号的差异表示对API的更改，版本号的每个部分表示更改的范围，如下所示。
...
.Note
====
以下规则适用于OpenXR版本1.0或更高版本。
OpenXR的预发行版本可能会使用不同的规则进行版本操作。
====
...
补丁程序编号的差异表明，通常修改了规格或标头的某些部分，通常是为了修复错误，
并且可能：对现有功能的行为产生影响。 补丁版本编号的差异必须：既不影响两个版本
之间的完整兼容性也不反向兼容，也可以：它在API中添加了其他接口。
运行时间可能：使用补丁版本编号来确定是否启用了影响功能的实现更改，例如错误修复。
Runtimes应该：记录与补丁版本相关的任何更改。 应用程序开发人员应：在使用新版本编译后，
对其支持的所有运行时间进行重新测试。
...
次要版本编号的差异表明已经添加了一些新功能。 这通常包括标题中的新接口，
可能：还包括行为更改和错误修复。 功能可能：在较小的修订中被弃用，但必须：不删除。 
当引入新的次要版本时，补丁版将重置为0，每个次要修订版都保持自己的补丁版本集。 
次要版本编号的差异应：不影响向后兼容性，而是会影响完全兼容性。
...
主要版本编号的差异表明对API的一系列更改，可能包括新功能和标题接口，行为变化，
删除不弃用的功能，修改或直接替换任何功能，因此很可能会破坏兼容性。 
主要版本编号的差异通常需要对应用程序代码进行重大修改，以使其正常运行。
...
// version numbers:
下表尝试详细说明不得更新时可能发生的更改（指示必须更新下一个版本号），以更新为任何专业，次要或补丁版本号：
...
// |=======================================================================
|=======================================================================
| Extensions Added/Removed(添加/删除扩展) {set:cellbgcolor:white}
   | 可以在此补丁级别上添加或删除扩展名。
| Specification-Optional Behavior Changed(改变规范-可选行为)
   | 此规范中规定的一些可选行为已改变。 
     通常，这将涉及具有规范标记的行为的变化：或可能： 
     例如，以前未验证特定用例的运行时间现在可以开始验证该用例。
| Specification-Required Behavior Changed(改变规范-必需行为)
   | 此规范要求的运行时间行为可能：已更改。 
     例如，以前的可选验证可能：现在已成为运行时间的强制性。
| Core Interfaces Added(添加了核心接口)
   | 在此级别的修订版中，新的接口可能：已添加到此规范中（以及OpenXR标头文件）。
| Weak Deprecation(弱废弃)
   | 界面可能：在此级别上被贬低。 这可能是：如果现在有一种更好的方法来完成同一件事，
     就会发生。 进行此通话的应用程序的行为应与贬值之前相同，但是遵循新路径可能更具性能，
     潜在或以其他方式产生更好的结果。 一些运行时间可能：选择发出时间警告，
     即该功能已被贬低，并且将来可能会被强烈贬值或删除。
| Strong Deprecation(强废弃)
   | 界面可能：在此级别上已强烈弃用。 这意味着界面必须：仍然存在（因此，
     针对它编译的应用程序仍将运行），但是它可能：现在是一个no-op，
     或者可能：它的行为已发生了显着更改。 可能：不再需要此功能，
     或者它的功能已被另一个调用所包含。 这不应该打破应用程序，
     但是某些行为可能是不同的或意外的。
| Interfaces Changed/Removed(改变/移除接口)
   | 接口可能：在此级别上已更改（具有不同的参数或返回类型）。
     接口或功能可能：也已完全删除。
     几乎可以肯定的是，需要重建申请。
|=======================================================================
...
该API使用字符串作为某些功能的输入和输出。
除非另有说明，否则所有此类字符串均为代码：NULL终止UTF-8
编码的情况敏感字符阵列。

[[threading-behavior]]
=== Threading Behavior(线程行为)
...
OpenXR API旨在在多个主机线程上使用时提供可扩展性能。
所有函数必须：支持从多个线程同时调用，但是将某些参数或参数组件定义为外部同步。
这意味着呼叫者必须：确保在给定时间使用此参数不超过一个线程。
...
// barrier if needed.
更确切地说，功能使用简单商店来更新代表对象的软件结构。 
声明为外部同步的参数可能：在函数执行期间的任何时间都会更新其软件结构。 
如果两个函数在同一对象上运行，并且至少一个函数声明要外部同步的对象，
则呼叫者必须：不仅保证函数不同时执行，而且两个函数也由适当的函数分开 如果需要，记忆障碍。
...
OpenXR API不能同时使用运行时明确识别，也不需要对多个进程的支持，也不会阻止运行时提供此类支持。
...
OpenXR运行时是实现OpenXR API的软件。 
可能会：系统上安装了一个以上的OpenXR运行时，
但是在任何给定时间只能活跃一个运行时。
...
OpenXR是一种可扩展的API，可以：通过添加新功能而增长。 
与其他Khronos API相似，可以使用扩展：用于公开新的OpenXR函数或修改现有OpenXR函数的行为。 
扩展是可选的，因此必须：在提供扩展功能之前，由应用程序启用。 
由于扩展是可选的，因此可以：仅在运行时间，图形平台或操作系统的子集中实现。
因此，应用程序应：首先查询在启用之前可以使用哪些扩展名。 
 
该应用程序使用Flink：XrenumerateInstanceExtensionProperties函数查询扩展名的可用列表。
一旦应用程序确定了支持哪些目标扩展，它可以：在呼叫中启用其中的某些子集：XRCREATEATEINSTANCE。
  
OpenXR扩展名具有独特的名称，可以传达有关提供什么功能的信息。 名称具有以下格式：

.Extension Name Formatting(扩展名字格式)
// ****
...
// ****
****
* 前缀“代码：xr_”将其识别为OpenXR扩展程序
* 供应商标签的字符串标识符，该标识符对应于公司或集团公开扩展名。
   供应商标签必须：仅使用大写字母和十进制数字。 一些示例包括：
** 针对Khronos扩展名的“代码：KHR”，由多个供应商支持。
** 多个供应商支持的非Khronos扩展名的“代码：ext”。
* 下划线“代码：_”。
* 一个唯一识别扩展名的字符串。
   字符串是子字符串的化合物，必须：仅使用小写字母和十进制数字。
   用单个下划线界定子字符串。
****
...
// composition layers.
例如：`<< xr_khr_composition_layer_cube >>`是由Khronos（KHR）OpenXR工作组创建的OpenXR扩展程序，以支持Cube Coble层。
...
OpenXR 被设计为分层 API，这意味着用户或应用程序可以： 在应用程序和运行时实现之间插入 API 层。 
这些 API 层通过拦截来自上层的 OpenXR 函数然后执行与没有该层时不同的操作来提供额外的功能。 
在最简单的情况下，该层仅使用相同的参数向下调用下一层，但更复杂的层可能会实现其下层或运行时中
不存在的 API 功能。 这种机制本质上是一种架构的“函数填充”或“拦截”功能，它被设计到 OpenXR 中，
旨在取代更多非正式的“挂钩”API 调用方法。

==== Examples of API Layers(API层的例子)
...
OpenXR 采用的分层 API 方法允许在“验证”层中对正确的 API 使用进行昂贵的验证。 
该层允许应用程序开发人员在激活验证层的情况下开发他们的应用程序，以确保应用程序正确使用 API。 
验证层确认应用程序已正确设置对象状态，已为每个功能提供所需数据，确保所需资源可用等。
如果验证层检测到问题，它会发出错误消息，可以记录或 由应用程序通过回调捕获。 
开发人员在确定应用程序无误后，关闭验证层，让应用程序在生产环境中运行，而无需重复产生验证费用。

===== API Logging Layer(API日志层)
...
API 层的另一个示例是 API 日志记录层，
它简单地将所有 API 调用序列化为文本格式的输出接收器，
包括打印出参数值和结构内容。

===== API Trace Layer(API跟踪层)
...
// a replay program.
相关的 API 跟踪层生成一个跟踪文件，其中包含提供给 API 的所有信息，以便重放程序可以回放跟踪文件。

==== Naming API Layers(API层命名方式)
...
为了组织 API 层名称并防止 API 层名称命名空间中的冲突，API 层必须： 使用以下约定命名：

```
XR_APILAYER_<VENDOR-TAG(供应商标记)>_short_name
```
...
// namespace.
供应商负责向 OpenXR 工作组注册供应商标签，就像实现开发者一样，他们必须维护自己的供应商名称空间。

// Example of an API layer name produced by the Acme company for the "check
// best practices" API layer:
Acme 公司为“检查最佳实践(check best practices)”API 层生成的 API 层名称示例：

```
XR_APILAYER_ACME_check_best_practices
```

==== Activating API Layers(激活 API 层)

===== Application Activation(应用程序激活)
...
// instance.
应用程序可以： 通过调用 flink:xrEnumerateApiLayerProperties 函数来获取可用 API 层列表，从而确定它们可用的 API 层。
然后，应用程序可以： 从该列表中选择所需的 API 层，并在创建实例时将它们提供给 flink:xrCreateInstance 函数。
...
应用程序用户或执行系统集成商或系统管理员等角色的用户可以： 配置系统以激活 API 层，而无需应用程序参与。 
这些平台相关的步骤可能：包括安装 API 层相关文件、设置环境变量或其他平台特定的操作。 
可用于以这种方式配置 API 层的选项也取决于平台和/或运行时。

==== API Layer Extensions(API 层扩展)
...
// layer.
API 层可以： 实现底层运行时可能支持或不支持的 OpenXR 功能。 为了公开这些新功能，
API 层必须以 OpenXR 
// <<extensions,extension>> 
<<extensions,扩展>> 
的形式公开此功能。 
它必须： 不公开没有关联扩展的新 OpenXR 函数。

例如，OpenXR API 日志记录 API 层可能会公开一个 API 函数，以允许应用程序仅为其执行的一部分打开日志记录。 
由于新功能必须： 通过扩展公开，供应商创建了一个名为“XR_ACME_logging_on_off”的扩展来包含这些新功能。 
应用程序应该：查询 API 层是否支持扩展，然后，仅当存在时，在 flink:xrCreateInstance 期间按名称启用扩展和 API 层。 

要找出 API 层支持哪些扩展，应用程序必须： 首先通过调用 flink:xrEnumerateApiLayerProperties 验证 API 层是否存在于当前系统上。 
在验证存在感兴趣的 API 层后，应用程序应：调用 flink:xrEnumerateInstanceExtensionProperties 并提供 API 层名称作为第一个参数。 
这将返回在该 API 层内部实现的扩展列表。
...
// described here.
类型别名是指元素的实际类型与声明的类型不匹配的情况。 
某些 C 和 C++ 编译器可以配置为假设实际类型与声明的类型相匹配，并且可以在常见优化级别默认配置。 
没有这个，否则可能会发生未定义的行为。 此编译器功能通常称为“严格别名”，通常可以通过编译器选项启用或禁用。 
OpenXR 规范不支持严格的别名，因为在某些情况下，应用程序会故意提供一个类型与声明类型不同的结构。 
例如，slink:XrFrameEndInfo::pname:layers 
是一个类型为 code:const slink:XrCompositionLayerBaseHeader code:* code:const 的数组。 
但是，数组必须： 属于特定层类型之一，例如 slink:XrCompositionLayerQuad。 
类似地，flink:xrEnumerateSwapchainImages 接受一个 slink:XrSwapchainImageBaseHeader 的数组，
而实际传递的类型必须：是一个类型的数组，例如 slink:XrSwapchainImageVulkanKHR。 
为使 OpenXR 正常工作，编译器必须： 支持此处描述的类型别名。
...
为方便起见，在规范时已知的一些类型和指针已使用 dlink:XR_MAY_ALIAS 
定义注释了不同类型的别名值。 如果在包含 OpenXR 标头之前未定义此宏，
并且使用了足够新的 Clang 或 GCC 编译器，则会将其定义为特定于编译器的属性注释，
以通知这些编译器这些指针可能存在别名。 但是，不能保证所有别名类型或指针都已使用此宏正确标记，
因此如果您选择（风险自负）允许编译器执行基于类型的别名分析，仍建议进行彻底测试。
...
有效用法定义了一组必须满足的条件： 为了在应用程序中实现明确定义的运行时行为。 
这些条件仅取决于 API 状态，以及使用受条件约束的参数或对象。 

一些有效的使用条件依赖于运行时限制或功能可用性。 可以根据 API 对这些限制和
功能的最小或最大支持值或其他已知值的某些子集来验证这些条件。 

有效的使用条件应该： 适用于函数或结构，在执行应用程序期间可以知道有关条件的完整信息。 
这使得验证 API 层或 linter 可以： 在指定这些语句时直接针对这些语句编写。

[[implicit-valid-usage]]
==== Implicit Valid Usage(隐式有效用法)
...
一些有效的使用条件适用于 API 中的所有函数和结构，除非针对特定函数或结构体。
这些条件被认为是隐含的。 
隐含的有效使用条件在下面详细描述。

[[valid-usage-for-object-handles]]
===== Valid Usage for Object Handles(对象句柄的有效用法)
...
作为对象句柄的函数的任何输入参数必须： 是有效的对象句柄，除非另有说明。
当且仅当满足以下条件时，对象句柄才有效：

.Object Handle Validity Conditions(对象句柄有效性条件)
// ****
...
// ****
****
* 它已由先前成功调用创建或分配应用程序接口,
* 它没有被之前对 API 的调用销毁，并且
* 它的父句柄也是有效的。
****
...
// value code:0.
在某些上下文中，对象句柄是可选的：或者未指定。
在这些情况下，API 使用 dlink:XR_NULL_HANDLE，它具有整数值 
code：0。

[[valid-usage-for-pointers]]
===== Valid Usage for Pointers(指针的有效用法)
...
任何作为指针的参数必须： 当规范指示运行时使用指针时，它是一个有效的指针。 
当且仅当指针指向包含函数期望的数字和类型值的内存时，指针才有效，
并且通过指针访问的所有基本类型（例如，作为数组的元素或作为结构的成员）满足 主机处理器的对齐要求。

[[valid-usage-for-enumerated-types]]
===== Valid Usage for Enumerated Types(枚举类型的有效用法)
...
枚举类型的任何参数必须： 是该类型的有效枚举。
当且仅当枚举数被定义为相关枚举类型的一部分时，枚举数才有效。

[[valid-usage-for-flags]]
===== Valid Usage for Flags(标记的有效用法)
...
// bitmasks.
位掩码被传递给许多函数和结构以紧凑地表示选项，并存储在由 basetype:XrFlags64 类型定义的内存中。 
但是API并没有直接使用basetype:XrFlags64类型。 相反，使用了 stext:Xr*Flags 类型，
它是 basetype:XrFlags64 类型的别名。 API 还定义了一组用于设置位掩码的常量位定义。
...
API 中使用的任何 stext:Xr*Flags 成员或参数必须： 是位标志的有效组合。
有效组合是零或按位代码：有效位标志的或。
当且仅当：

.Bit Flag Validity(位标志有效性)
// ****
...
// ****
****
* 位标志是由与 stext:Xr*Flags 成员或参数相同的 stext:Xr*Flags 类型定义的常量位定义之一。 有效的标志值也可以由扩展定义。
* 该标志在使用它的上下文中是允许的。 例如，在某些情况下，某些位标志或位标志的组合是互斥的。
****

[[valid-usage-for-structure-types]]
===== Valid Usage for Structure Types(结构类型的有效用法)
...
作为包含 pname:type 成员的结构的任何参数必须： 具有 pname:type 的值，
该值是与结构类型匹配的有效 elink:XrStructureType 值。 作为一般规则，
此值的名称是通过获取结构名称，去除前导代码：Xr，在每个大写字母前加下划线，
将整个结果字符串转换为大写，并在其前加上“XR_TYPE_”来获得的。 

此规则的唯一例外是 API 和操作系统名称，它们以产生更具可读性的值的方式转换：

.Structure Type Format Exceptions(结构类型格式异常)
****
* OpenGL => _OPENGL
* OpenGLES => _OPENGL_ES
* EGL => _EGL
...
===== Valid Usage for Structure Pointer Chains(结构指针链的有效用法)
...
任何包含 code:void* pname:next 成员的结构必须： 具有 pname:next 的值，
该值要么是 code:NULL，要么指向还包含 pname:type 和 pname:next 成员值的有效结构。 
由 pname:next 指针连接的结构集称为 pname:next 链。 

为了在 pname:next 链中使用由扩展定义的结构类型，正确的扩展必须： 
之前在 flink:xrCreateInstance 期间启用。 运行时必须： 
忽略 pname:next 链中所有无法识别的结构，包括那些与尚未启用的扩展关联的结构。 

核心 OpenXR 规范中描述了链中使用的一些结构，并在成员描述中提到。 
本文档中描述的用于链中的任何结构都在它们链接到的结构的隐式有效用法中
的“另请参阅”列表中提及。 大多数链式结构都与扩展相关联，并在基本 OpenXR 规范中进行了描述
ifndef::HAS_DEPRECATED_EXTENSIONS[]
// <<extension-appendices-list, List of Extensions>>.
<<extension-appendices-list, 扩展附录列表>>
endif::HAS_DEPRECATED_EXTENSIONS[]
...
// be unmodified.
特定于供应商的扩展可能：也可以在那里找到，或者可能：只能从供应商的网站或内部文档存储库中获得。

除非另有说明： 作为输出结构的链式结构可以： 由运行时修改，type 和 next 字段除外。
从任何函数返回时，链中的所有类型和下一个字段必须：未修改。

====== Useful Base Structures(有用的基础结构)
...
// structures:
为了方便需要遍历结构指针链的运行时和层，OpenXR API 提供了以下基本结构：
...
slink:XrBaseInStructure 结构定义为：
include::{generated}/api/structs/XrBaseInStructure.txt[]

.Member Descriptions(成员描述)
// ****
...
// ****
****
* pname:type 是这个结构的 elink:XrStructureType。 此基本结构本身没有关联的 elink:XrStructureType 值。
* pname:next 是 code:NULL 或指向结构链中下一个结构的指针。核心 OpenXR 中没有定义此类结构。
****

// slink:XrBaseInStructure can be used to facilitate iterating through a
// read-only structure pointer chain.
slink:XrBaseInStructure 可用于促进只读结构指针链的迭代。
--
...
slink:XrBaseOutStructure 结构定义为：
...
//   No such structures are defined in core OpenXR.
* pname:type 是这个结构的 elink:XrStructureType。 此基本结构本身没有关联的 elink:XrStructureType 值。
* pname:next 是 code:NULL 或指向结构链中下一个结构的指针。
   核心 OpenXR 中没有定义此类结构。
****
